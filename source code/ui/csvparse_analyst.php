...
while( $nxt_yr_date <= $date )
{
	...
	$query = "
	SELECT count(sentiment) as c2, screen_name
	FROM `tweet_tags`,company_names, tweets,followers
	where sentiment='b'
	and tag_id=1
	and company_id=tag_id 
	AND tweets.tweet_id = tweet_tags.tweet_id
    AND tweets.user_id=followers.user_id
	and followers.screen_name='tips2trade'
	and valid_till='".$date."'
	";
	...
	//echo $buy.'<br>';
	$query = "
	SELECT count(sentiment) as c1 ,screen_name
	FROM `tweet_tags`,company_names, tweets,followers
	where sentiment='s'
	and tag_id=1
	and company_id=tag_id 
...
fclose($file);
echo "(";
if (($handle = fopen("analyst.csv", "r")) !== FALSE) {
	$row = 0;
	$days_array = array();
